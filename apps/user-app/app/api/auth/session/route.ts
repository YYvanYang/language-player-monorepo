...
const sessionOptions: SessionOptions = {
  ...
  password: process.env.USER_SESSION_SECRET!, // MUST be set in .env.local/.env.production, complex, >= 32 chars
  cookieOptions: {
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    user: { id: session.userId /*, add other needed fields like name/role if stored */ },
